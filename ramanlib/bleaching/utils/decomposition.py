    ...
    def __init__(
        ...
        if basis_type == "free":
            ...
            if initial_poly_coeffs is not None:
                # Use provided polynomial coefficients directly (avoids double polynomial fitting)
                if isinstance(initial_poly_coeffs, torch.Tensor):
                    poly_tensor = initial_poly_coeffs.detach().clone().to(device)
                else:
                    poly_tensor = torch.tensor(
                        initial_poly_coeffs, dtype=torch.float32, device=device
                    )
                ...
                # WARNING: This re-fits polynomials which may introduce error
                assert (
                ...
                # Use shared polynomial fitting function
                coeffs_array, _, _ = fit_polynomial_bases(
            ...
            # Precompute Vandermonde matrix using shared function
...
def extract_initial_params_from_dataset(
    ...
    # Extract polynomial coefficients if available (for polynomial basis mode)
    if "poly_coeffs_gt" in dataset:
        poly_coeffs = dataset["poly_coeffs_gt"].values
        init_params["initial_poly_coeffs"] = torch.tensor(
            poly_coeffs, dtype=torch.float32, device=device
        )
...
def fit_physics_model(
    ...
    freeze_decay_rates: bool = False,
    freeze_abundances: bool = False,
    freeze_raman: bool = False,
    polynomial_degree: int = 8,
    ...
    # Handle initial_poly_coeffs conversion
    if initial_poly_coeffs is not None:
        if isinstance(initial_poly_coeffs, np.ndarray):
            initial_poly_coeffs = torch.tensor(
                initial_poly_coeffs, dtype=torch.float32, device=device
            )
        elif isinstance(initial_poly_coeffs, torch.Tensor):
            initial_poly_coeffs = initial_poly_coeffs.to(device)
        if verbose:
            print(f"✓ Using polynomial coefficients directly (shape: {initial_poly_coeffs.shape})")
    ...
    if freeze_decay_rates:
        model.decay_rates_raw.requires_grad = False
        if verbose:
            print("Decay rates frozen (fixed to initialization)")
    if freeze_abundances:
        model.abundances_raw.requires_grad = False
        if verbose:
            print("Abundances frozen (fixed to initialization)")
    if freeze_raman:
        model.raman_spectrum.requires_grad = False
        if verbose:
            print("Raman spectrum frozen (fixed to initialization)")

    # Check if any parameters are trainable
    trainable_params = list(filter(lambda p: p.requires_grad, model.parameters()))
    all_frozen = len(trainable_params) == 0

    if all_frozen:
        if verbose:
            print("All parameters frozen - skipping optimization")
        # Just compute initial loss and return
        with torch.no_grad():
            reconstruction = model()
            mse_loss = torch.mean((reconstruction[:first_times] - data_tensor[:first_times]) ** 2)
        history = {"loss": [mse_loss.item()], "mse": [mse_loss.item()]}
        if normalize_data and scale_factor != 1.0:
            model.raman_spectrum.data = model.raman_spectrum.data + np.log(scale_factor)
            model.abundances_raw.data = model.abundances_raw.data + np.log(scale_factor)
    ...
    if normalize_data and scale_factor != 1.0:
        with torch.no_grad():
            ...
            if verbose:
                print(
                    f"✓ Model parameters rescaled back to original units (x{scale_factor:.2f})"
                )
...
def visualise_decomposition(
    ...
    if reconstruction is None:
        ...
        for i in range(n_fluorophores):
            ...
            reconstruction = reconstruction + abundances[i] * decay[:, None] * bases[i, None, :]
