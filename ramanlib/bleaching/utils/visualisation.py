...
def plot_stacked_decomposition(
    ...
    ax.set_ylabel("Intensity")
    ax.set_title(
        f"Sample {sample_idx} at t={time_seconds:.2f}s - Stacked Decomposition"
    )
    ax.legend(loc="upper right")
    ax.grid(True, alpha=0.3)
...
if __name__ == "__main__":
    ...
    F0 = get_fluorophore_contribution(
        ds, sample_idx, fluorophore_idx=0, time_seconds=time_s
    )
    ...
    if HAS_PLOTLY:
        fig_3d = visualise_decomposition_3d(
            ds, sample_idx=0, subsample_wn=5, subsample_time=2
        )
...
def visualise_decomposition_3d(
    ...
    if "sample" in ds["fluorophore_bases_gt"].dims:
        bases = (
            ds["fluorophore_bases_gt"].isel(sample=sample_idx).values
        )  # (n_fluorophores, n_wn)
    else:
